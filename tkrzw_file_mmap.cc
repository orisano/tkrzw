...
class MemoryMapParallelFileImpl final {
 ...
 private:
  ...
  sf::contention_free_shared_mutex<> mutex_;
};
...
class MemoryMapParallelFileZoneImpl final {
 public:
  ...
  ~MemoryMapParallelFileZoneImpl();
  void SetLockedMutex(sf::contention_free_shared_mutex<>* mutex);
...
Status MemoryMapParallelFileImpl::Synchronize(bool hard, int64_t off, int64_t size) {
  ...
  std::lock_guard<sf::contention_free_shared_mutex<>> lock(mutex_);
  Status status(Status::SUCCESS);
...
Status MemoryMapAtomicFileImpl::GetSize(int64_t* size) {
  std::shared_lock<sf::contention_free_shared_mutex<>> lock(mutex_);
  if (fd_ < 0) {
    return Status(Status::PRECONDITION_ERROR, "not opened file");
  }
  *size = file_size_;
